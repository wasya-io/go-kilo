...
MetricsEnabled        bool // パフォーマンスメトリクスの有効化
}
...
// KILO_METRICS_ENABLED環境変数から設定を読み込む
if metrics := os.Getenv("KILO_METRICS_ENABLED"); metrics != "" {
config.MetricsEnabled = metrics != "0" && metrics != "false"
}
